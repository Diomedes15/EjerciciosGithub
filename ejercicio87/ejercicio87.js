...
const figuras = ["circulo", "cruz", "triangulo", "cuadrado", "rombo", "estrella", "corazon", "medialuna"];
const figurasDuplicadas = [];
let numerosAplicados = [];
figuras.forEach(figura => {
    figurasDuplicadas.push(figura, figura);
});
let numTarjetas = figurasDuplicadas.length;
while (numTarjetas > 0) {
    const numeroAleatorio = Math.floor(Math.random() * figurasDuplicadas.length);
...
tarjetas.forEach(tarjeta => {
    tarjeta.addEventListener("click", () => {
        ...
        if (todasGiradas)
            resultado.textContent = "¡Felicidades! Has ganado";
    });
});
