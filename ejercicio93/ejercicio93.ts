...
const texto = document.getElementById("texto") as HTMLHeadingElement;
const circulo = document.getElementById("circulo") as HTMLDivElement;

let combinaciones: number[] = [];
let combinacionesJugador: number[] = [];
let ronda : number = 0;
...
function nuevaRonda(){
    ...
    const numeroAleatorio: number = Math.floor(Math.random() * botones.length);
