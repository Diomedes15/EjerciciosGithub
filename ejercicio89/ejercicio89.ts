const inputTexto = document.getElementById("intro") as HTMLInputElement;
const pantalla = document.getElementById("texto") as HTMLParagraphElement;
inputTexto.focus();

let tiempoInicio: number | null = null;
let tiempoFin: number | null = null;
...
const palabrasElegidas: string[] = [];

while (palabrasElegidas.length < 50) {
    const numeroAleatorio = Math.floor(Math.random() * palabras.length);
    if (!palabrasElegidas.includes(palabras[numeroAleatorio])) palabrasElegidas.push(palabras[numeroAleatorio]);
}
...
const letras = pantalla.querySelectorAll<HTMLSpanElement>("span");
